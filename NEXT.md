...
| Pi workers | Second harness implemented 2026-09-15 on branch `pi-workers` (`tools/lab-worker-pi`, `tools/pi/lab-worker.js`, `tools/lab_pi.py`, `scripts/llm-server.sh`); acceptance section P (fake pi) and a node test of the extension; live smoke: gpt-oss-20b through llama.cpp answered a 3-turn tool-calling prompt under the extension | First probe `../labloop-pi-probe-20260915` (MNIST as M1, greedy, 5 candidates, gpt-oss-20b on GPU 0, training on GPU 1) launched 2026-09-15 16:20 | Read the probe: does an open model meet the worker contract inside 40 turns at all; then Qwen3.8-27B; merge to main after the read (`docs/pi-probe-20260915.md`) |
