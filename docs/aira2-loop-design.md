# The AIRA₂ loop — design and history

The design that replaced the phase machine, its milestones with their kill criteria,
and the dated implementation notes. Live status and the next work are in `NEXT.md`;
this file is the record. Section numbers (§1–§9) are cited from the code and docs.
...
intent. NEXT.md's status table is authoritative for progress; old examples are not
...
**M1 — Real worker, one GPU (≈ 3 days). DONE, see NEXT.md.**
...
completed, greedy comparison pending (see NEXT.md).**
